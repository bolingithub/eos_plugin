    ...
    public DataParam(String value, DataType type, Action action) {
        ...
        if (type == DataType.asset || type == DataType.symbol) {
            if (action == action.up) {
                ...
                this.value = vs[0] + " " + action.getCode().replace("${precision}", String.valueOf(precision)).replace("${value}", vs[1]);
            } else if (action == action.transfer || action == action.delegate || action == action.close) {
                String vs[] = value.split(" ");
                if (vs.length < 2) {
                    throw new EException("error", "quantity error");
                }
