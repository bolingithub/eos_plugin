package android.src.main.kotlin.com.develop.wallet.eos.utils.ese;

import com.develop.wallet.eos.crypto.utils.ByteUtils;
import com.develop.wallet.eos.utils.Hex;
...
public class Ese {
    ...
    public static String parseUpData(String upfrom, String upto, String value) {
        DataParam[] datas = new DataParam[]{new DataParam(upfrom, DataType.name, Action.up),
                new DataParam(upto, DataType.name, Action.up),
                new DataParam(value, DataType.asset, Action.up)};
        byte[] allbyte = new byte[]{};
        for (DataParam dataParam : datas) {
            allbyte = ByteUtils.concat(allbyte, dataParam.seria());
        }
        // final byte [] b = allbyte.clone();
        // int[] a = IntStream.range(0, b.length).map(i -> b[i] & 0xff).toArray();
        // for(int i=1;i<=a.length;i++) {
